...
export default function Home() {
  ...
  // Pre-fill URL and action from query params (shareable links)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const shared = params.get("url");
    const sharedAction = params.get("action") as Action | null;
    if (shared) {
      setUrl(shared);
      setInputType("url");
      if (sharedAction && allActions.some(a => a.id === sharedAction)) {
        setAction(sharedAction);
      }
    }
  }, []);

  // Fetch scan status on mount — works for both signed-in and anonymous users
  useEffect(() => {
    fetch("/api/scan-status")
      ...
      .then((data) => {
        if (data.signedIn) {
          ...
          setIsSignedIn(false);
          if (typeof data.anonScansLeft === "number") {
            setScansLeft(data.anonScansLeft);
          }
        }
      })
      .catch(() => {});
  ...
  return (
    <>
      ...
      <main className="relative z-10 flex flex-col items-center px-3 sm:px-4 pt-20 sm:pt-24 pb-20 min-h-screen">

        {/* Hero */}
        <div className="relative text-center mb-8 sm:mb-10 max-w-2xl">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-violet-500/30 bg-violet-500/10 text-violet-300 text-xs mb-5 sm:mb-6 font-medium">
            ✨ Free to try. No account needed.
          </div>
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold text-white mb-4 sm:mb-5 leading-[1.1] tracking-tight">
            Stop scrolling.<br />
          ...
          <p className="text-white/50 text-sm sm:text-lg leading-relaxed max-w-lg mx-auto px-2">
            Paste a YouTube video, website, PDF or image. Get a clear summary, flashcards, quiz, and more in seconds.
          </p>
        </div>

        {/* How it works */}
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-8 sm:mb-10 w-full max-w-2xl">
          {steps.map((s, i) => (
            <div key={s.title} className="flex-1 flex items-center gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl border border-white/10 bg-white/[0.03]">
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-violet-500/20 flex items-center justify-center text-xs font-bold text-violet-400 shrink-0">{i + 1}</div>
              <div>
                <p className="text-white text-xs sm:text-sm font-medium">{s.title}</p>
                <p className="text-white/40 text-[11px] sm:text-xs">{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Main Card */}
        <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-white/[0.04] p-4 sm:p-6 mb-4 shadow-2xl">

          {/* Input toggle */}
          <div className="flex gap-2 mb-4 sm:mb-5 p-1 rounded-xl bg-white/5 border border-white/10">
            {(["url", "file"] as InputType[]).map((t) => (
              <button key={t} onClick={() => { setInputType(t); resetForNewInput(); setFile(null); setUrl(""); }}
                className={`flex-1 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all cursor-pointer ${inputType === t ? "bg-violet-600 text-white shadow" : "text-white/50 hover:text-white"}`}>
          ...
          {inputType === "url" && (
            <div>
              <input type="text" value={url} onChange={(e) => setUrl(e.target.value)}
                ...
                placeholder="Paste a YouTube link, website URL..."
                className="w-full px-3 sm:px-4 py-3 sm:py-3.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder-white/25 text-sm outline-none focus:border-violet-500/60 transition-colors" />
              {!url && (
                <div className="flex items-center gap-1.5 sm:gap-2 mt-2 flex-wrap">
                  <span className="text-[10px] sm:text-[11px] text-white/20">Try:</span>
                  {[
                    ...
                    <button key={ex.label} onClick={() => setUrl(ex.url)}
                      className="text-[10px] sm:text-[11px] text-violet-400/70 hover:text-violet-300 border border-violet-500/20 hover:border-violet-500/40 px-2 py-0.5 rounded-full transition-all cursor-pointer">
                      {ex.label}
                    </button>
                  ))}
                </div>
          ...
          {inputType === "file" && (
            <div onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)}
              ...
              className={`w-full min-h-[100px] sm:min-h-[110px] flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed cursor-pointer transition-all ${dragging ? "border-violet-500 bg-violet-500/10" : "border-white/15 hover:border-violet-500/40 bg-white/[0.02]"}`}>
              <input ref={fileRef} type="file" className="hidden" accept=".pdf,.txt,.md,image/*"
                onChange={(e) => e.target.files?.[0] && setFile(e.target.files[0])} />
              {file ? (
                <div className="text-center px-4">
                  <p className="text-violet-300 font-medium text-sm truncate max-w-[250px]">{file.name}</p>
                  <p className="text-white/30 text-xs mt-1">{(file.size / 1024).toFixed(1)} KB · tap to change</p>
                </div>
              ) : (
                <>
                  <span className="text-2xl sm:text-3xl">📎</span>
                  <p className="text-white/50 text-xs sm:text-sm font-medium">Drop your file here or tap to browse</p>
                  <p className="text-white/25 text-[11px] sm:text-xs">PDF, PNG, JPG, TXT supported</p>
                </>
              )}
            </div>
          )}
            ...
            <div className="grid grid-cols-4 gap-1.5 sm:gap-2">
              {analyzeActions.map((a) => (
                <button key={a.id} onClick={() => setAction(a.id)}
                  className={`flex flex-col items-center gap-1 sm:gap-1.5 py-2.5 sm:py-3 px-1 sm:px-2 rounded-xl border text-[10px] sm:text-xs transition-all cursor-pointer ${action === a.id ? "border-violet-500/60 bg-violet-500/15 text-violet-300" : "border-white/10 bg-white/[0.03] text-white/40 hover:text-white/70 hover:border-white/20"}`}>
          ...
          <div className="mt-2 sm:mt-3">
            ...
            <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
              {studyActions.map((a) => (
                <button key={a.id} onClick={() => setAction(a.id)}
                  className={`flex flex-col items-center gap-1 sm:gap-1.5 py-2.5 sm:py-3 px-1 sm:px-2 rounded-xl border text-[10px] sm:text-xs transition-all cursor-pointer ${action === a.id ? "border-pink-500/60 bg-pink-500/15 text-pink-300" : "border-white/10 bg-white/[0.03] text-white/40 hover:text-white/70 hover:border-white/20"}`}>
                  <span className="text-lg sm:text-xl">{a.icon}</span>
                  <span className="font-medium">{a.label}</span>
                </button>
              ))}
            ...
            <div className="flex gap-2 mt-2 sm:mt-3">
              {(["brief", "detailed"] as SummarizeMode[]).map((mode) => (
                <button key={mode} onClick={() => setSummarizeMode(mode)}
                  className={`flex-1 py-2 rounded-lg text-[11px] sm:text-xs font-medium transition-all border cursor-pointer ${summarizeMode === mode ? "border-violet-500/60 bg-violet-500/15 text-violet-300" : "border-white/10 bg-white/[0.03] text-white/40 hover:text-white/60"}`}>
          ...
          {action === "search" && (
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              ...
              className="w-full mt-3 sm:mt-4 px-3 sm:px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-white/25 text-sm outline-none focus:border-violet-500/60 transition-colors" />
          )}

          {/* Error */}
          {error && (
            <div className="mt-3 sm:mt-4 px-3 sm:px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/20">
          ...
          <button onClick={handleSubmit} disabled={loading}
            className="mt-4 sm:mt-5 w-full py-3 sm:py-3.5 rounded-xl bg-gradient-to-r from-violet-600 to-pink-600 text-white font-semibold text-sm hover:opacity-90 disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 shadow-lg">
            {loading ? (
          ...
          {/* Scan status — shown for BOTH signed-in and anonymous users */}
          <div className="mt-3 flex items-center justify-center">
            {isSignedIn && scansLeft !== null && (
              <>
                {scansLeft > 0 ? (
                  <p className="text-xs text-white/30 text-center">
                    <span className="text-green-400 font-semibold">{scansLeft}</span> scan{scansLeft !== 1 ? "s" : ""} left today ·{" "}
                    <a href="/upgrade" className="text-violet-400 hover:underline">Upgrade for more</a>
                  </p>
                ) : pageTimer ? (
                  <div className="flex flex-col items-center gap-0.5">
                    <p className="text-[11px] text-white/25 uppercase tracking-wider">Scans reset in</p>
            ...
            {!isSignedIn && scansLeft !== null && (
              <p className="text-xs text-white/30 text-center">
                <span className="text-green-400 font-semibold">{scansLeft}</span> free scan{scansLeft !== 1 ? "s" : ""} left · <a href="/upgrade" className="text-violet-400 hover:underline">Sign in for more</a>
              </p>
            )}
          </div>
        </div>
        ...
        <details className="w-full max-w-2xl mb-4 group rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
          <summary className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-3.5 cursor-pointer select-none list-none">
            <div className="flex items-center gap-2">
              <span className="text-white/40 text-sm">ℹ️</span>
              <span className="text-white/50 text-[10px] sm:text-xs font-semibold uppercase tracking-wider">Requirements &amp; plan limits</span>
            </div>
            <span className="text-white/25 text-xs group-open:rotate-180 transition-transform duration-200 inline-block">▾</span>
          </summary>

          <div className="px-4 sm:px-5 pb-5 pt-1 space-y-5">
            {/* Supported formats */}
            <div>
            ...
            <div>
              ...
              <div className="rounded-xl border border-white/8 overflow-x-auto text-xs">
                <div className="grid grid-cols-5 bg-white/[0.03] border-b border-white/8 min-w-[340px]">
                  <div className="px-2 sm:px-3 py-2 text-white/25 font-medium"></div>
                  {["Free", "Starter", "Pro", "Unlimited"].map(p => (
                    <div key={p} className={`px-2 sm:px-3 py-2 text-center font-semibold ${p === "Pro" ? "text-violet-300" : p === "Unlimited" ? "text-amber-300" : "text-white/40"}`}>{p}</div>
                  ))}
                </div>
                {[
                  { label: "Scans / day", vals: ["4", "5", "20", "∞"] },
                  { label: "YT length", vals: ["20 min", "45 min", "3 hrs", "∞"] },
                  { label: "PDF size", vals: ["5 MB", "5 MB", "5 MB", "5 MB"] },
                  { label: "Price", vals: ["Free", "$3.99", "$8.99", "$15.99"] },
                ].map((row, i) => (
                  <div key={row.label} className={`grid grid-cols-5 min-w-[340px] ${i % 2 === 0 ? "" : "bg-white/[0.01]"} border-b border-white/5 last:border-0`}>
                    <div className="px-2 sm:px-3 py-2.5 text-white/35 font-medium">{row.label}</div>
                    {row.vals.map((v, j) => (
                      <div key={j} className={`px-2 sm:px-3 py-2.5 text-center ${j === 2 ? "text-violet-300 font-semibold" : j === 3 ? "text-amber-300 font-semibold" : "text-white/45"}`}>{v}</div>
                    ))}
                  </div>
                ))}
              </div>
              <p className="text-white/20 text-[10px] mt-2">
                Video limit enforced at scan time.{" "}
        ...
        {/* What works */}
        <div className="w-full max-w-2xl mb-8 rounded-2xl border border-white/10 bg-white/[0.02] p-4 sm:p-5">
            ...
            {extractedContent && (
              <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
                <div className="flex items-center gap-2 px-4 sm:px-5 py-3 border-b border-white/8">
                  ...
                  <p className="text-white/60 text-xs sm:text-sm font-medium">Ask anything about this content</p>
                  <span className="ml-auto text-[10px] text-white/20 uppercase tracking-wider">Free</span>
                </div>

                {chatMessages.length > 0 && (
                  <div className="px-3 sm:px-5 py-4 space-y-4 max-h-80 overflow-y-auto">
                    {chatMessages.map((msg, i) => (
                      <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[90%] sm:max-w-[85%] px-3 sm:px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${msg.role === "user"
                ...
                <div className="flex gap-2 p-2.5 sm:p-3 border-t border-white/8">
                  <input
                    type="text"
                    value={chatInput}
                    ...
                    className="flex-1 px-3 sm:px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder-white/25 text-sm outline-none focus:border-violet-500/50 transition-colors min-w-0"
                  />
                  <button
                    onClick={handleChat}
                    disabled={chatLoading || !chatInput.trim()}
                    className="px-3 sm:px-4 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-pink-600 text-white text-sm font-semibold hover:opacity-90 disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed transition-all shrink-0">
                    Ask
                  </button>
                </div>
              </div>
